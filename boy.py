...
class IdleState:
    def enter(boy, event):
        if event == RIGHT_DOWN:
            ...
            boy.velocity += 1
        elif event == SHIFT_DOWN or event == SHIFT_UP:
            return
        boy.timer = 1000
    ...
    def draw(boy):
        if boy.dir == 1:
            boy.image.clip_draw(boy.frame * 100, 300, 100, 100,
                                boy.x, boy.y)
        else:
            boy. image.clip_draw(boy.frame * 100, 200, 100, 100,
                                 boy.x, boy.y)
...
class RunState:
    ...
    def draw(boy):
        if boy.velocity == 1:
            boy.image.clip_draw(boy.frame * 100, 100, 100, 100,
                                boy.x, boy.y)
        else:
            boy.image.clip_draw(boy.frame * 100, 0, 100, 100,
                                boy.x, boy.y)
...
class Boy:
    ...
    def draw(self):
        ...
        debug_print('Velocity:' + str(self.velocity) + ' Dir:' + str(self.dir)
                    + ' Timer:' + str(self.timer) + ' Current_State:' + str(self.cur_state))
